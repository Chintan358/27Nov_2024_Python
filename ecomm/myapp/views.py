...
def orderplaced(request):
    if request.method=="GET":
        ...
        oid = f"EVERA_${random.randint(1000,9999)}_${request.user.id}"
        cartdata = Cart.objects.filter(user=request.user)
        order = UserOrder.objects.create(orderid=oid,user=request.user,date="2023-10-10",paymenttype=paymenttype,pid=paymentid,address_id=addressid)
        ...
        orderdata = UserOrder.objects.filter(pk=order.id)
        html_message = render_to_string('ordermail.html',{"orders":orderdata})
        plain_message = strip_tags(html_message)
        email_from = settings.EMAIL_HOST_USER
        recipient_list = [request.user.email]
        send_mail( "Order Confirmation", plain_message, email_from, recipient_list ,html_message=html_message)
